...
MU_WATER   = 0.02    # mm⁻¹  (must match config.py)
...
def hu_to_mu(hu: np.ndarray, mu_water: float) -> np.ndarray:
    """Convert HU values to linear attenuation coefficients (μ, mm⁻¹).
    Uses the same formula as projection.py: mu = clip((HU+1000)/1000 * mu_water, 0).
    """
    return np.clip((hu.astype(np.float32) + 1000.0) / 1000.0 * mu_water, 0.0, None)
...
def main():
    ...
    for split, case_list in splits.items():
        ...
        for case_id, fdk_path, nii_path in case_list:
            ...
            # Raw: FDK reconstruction — already (Z, Y, X), μ units
            fdk = np.load(fdk_path).astype(np.float32)

            # Label: NIfTI GT — nibabel returns (X, Y, Z); transpose to (Z, Y, X)
