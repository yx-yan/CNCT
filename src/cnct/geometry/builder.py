"""TIGRE cone-beam geometry construction for the ``cnct`` DL package.

Duplicated (intentionally, not shared) from
:mod:`cnct_dataprep.geometry.builder` so that the deep-learning package has
no runtime dependency on the data-preparation package. Implementations must
stay numerically identical — the DBP layer in :mod:`cnct.models` relies on
this builder producing the same geometry as the FDK forward projection.
"""
